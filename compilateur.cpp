...
void Error(string s) {
    cerr << "Erreur ligne " << lexer->lineno() << ": '" << lexer->YYText() << "' - ";
    cerr << s << endl;
    exit(-1);
}
...
enum TYPES Identifier(void) {
    if (!IsDeclared(lexer->YYText())) {
        cerr << "Variable non déclarée: " << lexer->YYText() << endl;
        exit(-1);
    }
...
void CaseStatement() {
    ...
    enum TYPES exprType = Expression();
    if (exprType != INTEGER && exprType != CHAR) {
        Error("Expression CASE doit être INTEGER ou CHAR");
    }
    
    if (current != KEYWORD || strcmp(lexer->YYText(), "OF") != 0) {
        Error("'OF' attendu après expression CASE");
    }
    current = (TOKEN)lexer->yylex();
    
    cout << "\tpop %rax\t# Valeur de l'expression CASE" << endl;
    cout << "CaseStart" << caseTag << ":" << endl;
    
    // Traitement des cas
    while (true) {
        // Liste de constantes
        set<string> caseConstants;
        do {
            if (current != NUMBER && current != CHARCONST) {
                Error("Constante attendue dans CASE");
            }
            caseConstants.insert(lexer->YYText());
            current = (TOKEN)lexer->yylex();
        ...
        if (current != COLON) Error("':' attendu après constantes");
        current = (TOKEN)lexer->yylex();
        
        // Générer les checks
        for (const auto& cst : caseConstants) {
            unsigned long long checkTag = ++TagNumber;
            cout << "\tcmp ";
            if (exprType == CHAR) cout << "$" << (int)cst[1] << ", %al" << endl;
            else cout << "$" << cst << ", %rax" << endl;
            
            cout << "\tje CaseMatch" << checkTag << endl;
        }
        
        cout << "\tjmp CaseNext" << caseTag << endl;
        
        // Bloc de code pour ce cas
        for (const auto& cst : caseConstants) {
            cout << "CaseMatch" << TagNumber - caseConstants.size() + 1 << ":" << endl;
        }
        Statement();
        cout << "\tjmp CaseEnd" << caseTag << endl;
        
        cout << "CaseNext" << caseTag << ":" << endl;
        
        // Vérifier si fin ou nouveau cas
    ...
    if (current != KEYWORD || strcmp(lexer->YYText(), "END") != 0) {
        Error("'END' attendu pour finir CASE");
    }
    current = (TOKEN)lexer->yylex();
    
    cout << "CaseEnd" << caseTag << ":" << endl;
}

// Statement := AssignementStatement|DisplayStatement| ... | CaseStatemnt
void Statement(void){
	if(current==KEYWORD){
